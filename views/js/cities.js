		...
		fields: {
			first_name: {
				validators: {
					stringLength: {
						min: 3,
						message: 'First name must have 3 characters'
					},
					notEmpty: {
					...
					regexp: {
						regexp: /^[a-z\s]+$/i,
						message: 'Name can consist only alphabets and spaces'
					}
				}
			},
			last_name: {
			...
			user_contact: {
				validators: {
					stringLength: {
						min: 8,
						max: 14,
						message: 'Contact number should contain min 8 and max 14 digit'
					},
					notEmpty: {
...
$(document).ready(function () {
	$('#restaurant_form').bootstrapValidator({
		// To use feedback icons, ensure that you use Bootstrap v3.1.0 or later
		feedbackIcons: {
			valid: 'glyphicon glyphicon-ok',
			invalid: 'glyphicon glyphicon-remove',
		...
		fields: {
			...
			restaurant_acc_holder_name: {
				validators: {
					stringLength: {
						min: 3,
						message: 'Account Name length should be at least 3'
					},
					notEmpty: {
					...
					regexp: {
						regexp: /^[a-z\s]+$/i,
						message: 'Account name can consist of only characters and spaces'
					}
				}
			},
			restaurant_name: {
			...
			restaurant_cost_two: {
				validators: {
					notEmpty: {
						message: 'Please enter average cost for 2 customers'
					},
					regexp: {
						regexp: /^[1-9]\d*(\.\d+)?$/i,
						message: 'Please enter correct cost'
					}
				}
			},
			restaurant_contact: {
			...
			restaurant_acc_number: {
				validators: {
					...
					regexp: {
						regexp: /^[0-9]{9,18}$/i,
						message: 'Account number should be minimum 9 and maximum 18 number'
					}
				}
			},
			restaurant_state: {
		...
		.on('success.form.bv', function (e) {
			$('#success_message').slideDown({ opacity: "show" }, "slow") // Do something ...
			$('#restaurant_form').data('bootstrapValidator').resetForm();
